...
ADDITIONAL OPTION: There is now a parallelised version called *wolPredictor_MANUAL_parallel.py* - it runs exactly the same as *wolPredictor_MANUAL.py* but without the `-s` shuffle option. It runs about 3x faster on my 4 core laptop. It produces some weirdly verbose output on the Anaconda Prompt consol screen but runs otherwise fine. I recomend this version.
