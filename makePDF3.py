...
def makePDF(f, gap, tix, spl):
    ...
    dat = np.genfromtxt(f, delimiter=',', dtype = 'U20', skip_header = 1) 
    wsp = np.unique(dat[:, 1])
    ...
    if tix == 1:
        ...
        plt.xlabel('Pre-defined species delimitation clusters')
        plt.xticks([], [])
    plt.ylabel('Number of correct predictions\n(uninfected vs. positive infections)')
    plt.yticks(ytix, np.absolute(ytix))

    for i in np.arange(0, data_shape[0]):
        ax.bar(np.arange(data_shape[1]), data[i], bottom=data_stack[i], color=cols[i],)

    #plt.show()
    fig.savefig(f.replace('_correctedWolPreds', '')[:-4] + '.pdf', dpi = 600)
    fig.savefig(f.replace('_correctedWolPreds', '')[:-4] + '.png', dpi = 600)
