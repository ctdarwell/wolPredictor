...
communities = 'fig.species' #column name of host community
...
tree = 'waspRaxml.tre' # waspRaxml.tre OR geigerDevtTree.tre
...
def addPredict(res, thresh, tmpDF):
    ...
    for comm in comms: #go thru fig host comms
        indvs = dat[np.where(dat[:, colmap.get('fig.species')] == comm)][:, colmap['NameOnPhylo']]
