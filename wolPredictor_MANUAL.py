...
if shuffle == 1: td2 = taxonDesignations.copy()
...
#vfunc fao addPredict
def f1(comm, x): return f"{comm[:3]}_{x}"
v1 = np.vectorize(f1, excluded=['comm']) #, otypes=['O']
...
def main():
    ...
    for delim in range(1, taxonDesignations.shape[1]): #Main pgm loop!!!!!
        ...
        if shuffle == 0: res = taxonDesignations[:, delim]
        else: #shuffle species clusters
            np.random.shuffle(td2[:, delim])
            res = td2[:, delim]
        df = addPredict(res, delim, tmpDF) #add wol strain predictions at this spp delim level
    ...
    if pdf == 1: #y/n write results figures
        tix = 0 #!!!!!!!!!!!!!!!!!!!!!!!!!RM TICKS FOR MANUAL - CHANGE ALT WOLPRED
...
def addPredict(res, nSpp, tmpDF):
    ...
    for comm in comms: #go thru fig host comms
        ...
        clusters = res[np.isin(taxa, indvs)]
        if np.unique(clusters).shape[0] > 1: grps = v1(comm=comm, x=clusters)
        else: grps = np.full((indvs.shape[0]), 'noWol')
        wolClades.extend(grps)
        
    cols.append('{}_noPurge'.format(degCols[nSpp])) #!!!
    taxaCols.append('{}_nSpp{}'.format(degCols[nSpp], str(np.unique(res).shape[0]))) #!!!
