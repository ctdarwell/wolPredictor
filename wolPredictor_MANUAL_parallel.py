...
def main():
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        results = executor.map(addPredict, clades, list(range(1, taxonDesignations.shape[1])))
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        pgeRes = executor.map(wolPurger, res, itertools.repeat(thresh2), itertools.repeat(cophen)) #purge wol at incremental thresholds at this spp delim

    purges = [p for p in pgeRes] #results from wolPurger
    
    #build up assigned dataframe
    cntr, cntr2, cntr3, l = 2, 3, 0, len(thresh2)
    for r in res:
        assigned[:, cntr] = r
        assigned[:, cntr2:cntr2 + l] = purges[cntr3]
        cntr2 += l + 1
        cntr += l + 1
        cntr3 += 1
